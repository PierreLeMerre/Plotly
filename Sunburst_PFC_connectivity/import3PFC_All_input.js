...
function importData(id, callback) {
	...
	var url = "http://localhost/PFC/data/structureData.json";
	$.ajax(url, {
		crossDomain: true,
	...
	//Input data
	apiQuery("http://localhost/PFC/data/PFC_All_input.json",processExpression);
	apiQuery("http://localhost/PFC/data/PFC_All_input.json",processNumber);
	apiQuery("http://localhost/PFC/data/PFC_All_input.json",processRatio);
	...
	function processNumber(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			number[e.structure_id] = String(e.number) +" (" +String(d3.round(e.number/17,0)) +")";
		}
		processData();
	}
